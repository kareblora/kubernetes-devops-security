pipeline {
  ...
  environment {
    ...
    applicationURL = "devsecops-demo-kar.eastus.cloudapp.azure.com"
    ...
    //    }       
      stage('Vulnerability Scan - Kubernetes') {
            steps {
              parallel(
                "OPA Scan": {
                  sh sh 'docker run --rm -v $(pwd):/project openpolicyagent/conftest test --policy opa-k8s-security.rego k8s_deployment_service.yaml'
                },
                "Kubesec Scan": {
                  sh "kubesec-scan.sh"
                },              
